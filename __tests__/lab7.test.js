describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    */
    ...
    for(let i of prodItemsData){
      if (i.title.length == 0) { allArePopulated = false; }
      if (i.price.length == 0) { allArePopulated = false; }
      if (i.image.length == 0) { allArePopulated = false; }
    }
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i of prodItems) {
      const shadowRoot = await i.evaluateHandle(el => el.shadowRoot);
      const button = await shadowRoot.$('button');
      await button.click();
    }

    const countCart = await page.$eval('#cart-count', el => el.textContent);
    expect(countCart).toBe('20');
  }, 20000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i of prodItems) {
      const buttonText = await page.evaluate((product) => {
        const shadow = product.shadowRoot;
